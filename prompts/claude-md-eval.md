...
5. 파일을 수정하지 말고 평가 결과, 개선 제안, 개선된 초안을 출력하라
...
> 근거: 에이전트는 지시를 따르지만, 아키텍처 설명이나 디렉토리 개요는 에이전트가 관련 파일을 찾는 속도를 높이지 못한다. [^1]
...
> 근거: 실제 CLAUDE.md 파일 2,303개를 분석한 결과, Claude Code 파일의 Flesch Reading Ease 중앙값은 16.6으로 법률 문서 수준이다. 짧고 명료한 파일이 더 안정적으로 준수된다. [^2]
...
> 근거: Claude는 CLAUDE.md를 `<system-reminder>`로 주입하되 "highly relevant to your task"가 아니라고 판단하면 무시한다. Claude Code 시스템 프롬프트에는 이미 약 50개의 지시사항이 포함되어 있다. [^3]

---

### 5. 계층 설계 (0~10점)
*(모노레포 또는 멀티 파일 구조인 경우)*
...
- 단일 레포이고 CLAUDE.md가 하나뿐이면 N/A 처리 (감점 없음)
...
> 근거: 오래된 빌드 명령이 문서에 있으면 에이전트가 그대로 따르다 실패한다. bootstrapping은 쉽지만 유지보수가 진짜 어렵다. [^4]
...
> 근거: `/init`으로 자동생성된 context file은 성공률을 평균 2~3% 낮추고 비용을 20% 이상 올린다. 단, 기존 문서화가 부족한 레포에서는 예외적으로 유효할 수 있다. [^1]
...
> 근거: 빈 파일에서 시작해 에이전트가 반복적으로 틀리는 패턴만 추가하는 방식이 가장 효과적이다. 에이전트가 한 번 틀린 것이 아닌, 반복적으로 틀리는 패턴이 확인됐을 때만 추가하라. [^5]
...
원본을 기반으로 위 피드백을 반영한 개선 버전 전체 작성
- 삭제한 내용: <!-- 삭제: [이유] --> 주석으로 표시
- 추가한 내용: ← NEW 태그 표시
```
